	...
	public void load(FMLInitializationEvent event) {
		if (event.getSide() == Side.CLIENT)
			Minecraft.getMinecraft().getRenderItem().getItemModelMesher()
					.register(block, 0, new ModelResourceLocation("LOZmod:OcarinaTime", "inventory"));
		block.setCreativeTab(LOZ.tab);
	}

	static {
		block = (new ItemocarinaTime());
	}

	static class ItemocarinaTime extends Item {

		public ItemocarinaTime() {
			setMaxDamage(0);
			maxStackSize = 1;
			setUnlocalizedName("ocarinatime");
			setRegistryName("ocarinatime");
