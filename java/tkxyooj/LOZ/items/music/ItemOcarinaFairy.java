...
public class ItemOcarinaFairy {

	public ItemOcarinaFairy() {
	}
	...
	public void load(FMLInitializationEvent event) {
		if (event.getSide() == Side.CLIENT)
			Minecraft.getMinecraft().getRenderItem().getItemModelMesher()
					.register(block, 0, new ModelResourceLocation("LOZmod:OcarinaFairy", "inventory"));
		block.setCreativeTab(LOZ.tab);
	}

	static {
		block = (new ItemocarinaFairy());
	}

	static class ItemocarinaFairy extends Item {

		public ItemocarinaFairy() {
			setMaxDamage(0);
			maxStackSize = 1;
			setUnlocalizedName("ocarinafairy");
			setRegistryName("ocarinafairy");
	    ...
	    protected void useInstrument(ItemStack stack, Entity entity, int cooldown)
	        {
	            ...
	            if(tag.getInteger(INSTR_COOLDOWN_KEY) == 0)
	            {
	                if(!entity.world.isRemote)
	                {
	                        ...
	                        doMagic(stack, (EntityLivingBase) entity);
	                }
	                tag.setInteger(INSTR_COOLDOWN_KEY, cooldown);
	            }
	    }
	    
	    public static final String healthmessage = "You have regain some health!";
	    public static final String noeffectmessage = "Healing magic has no effect!";

	    private void doMagic(ItemStack stack, EntityLivingBase entity) 
	    {
	    	float health = entity.getHealth();
	    	//if health is less than full, restore 4 hearts
	    	if(health >= 20)
	    	{
	    	   	entity.sendMessage(new TextComponentString(noeffectmessage));
	    	}
	    	else if (health < 20)
	    	{
	    		entity.heal(8);
	    		entity.sendMessage(new TextComponentString(healthmessage));
	    	}
		}

		ResourceLocation location = new ResourceLocation("LOZmod", "lozmod.music.sound.healing");
